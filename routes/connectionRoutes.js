  ...
  try {
    ...
    if (deletedConnection.status === "accepted") {
      if (deletedConnection.requester.toString() === currentUserId.toString()) {
        ...
        await User.findByIdAndUpdate(deleteRequestId, { $inc: { followersCount: -1 } });
      } else {
        await User.findByIdAndUpdate(currentUserId, { $inc: { followersCount: -1 } });
        await User.findByIdAndUpdate(deleteRequestId, { $inc: { followingCount: -1 } });
      }
    }
...
router.get("/fetch/connections", authMiddleware, async (req, res) => {
  try {
    ...
    const formattedAcceptedAsRequester = acceptedConnectionsAsRequester.map(conn => ({
      _id: conn._id,
      user: conn.recipient,
    ...
    const formattedAcceptedAsRecipient = acceptedConnectionsAsRecipient.map(conn => ({
      _id: conn._id,
      user: conn.requester,
      connectedAt: conn.updatedAt,
      type: "friend"
    ...
    const acceptedConnections = [...formattedAcceptedAsRequester, ...formattedAcceptedAsRecipient];
